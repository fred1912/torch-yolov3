...
class yoloCOCO(COCO):
    ...
    def __getitem__(self, index):
        ...
        label = [np.zeros((output_h, output_w,anchors_num, 5+num_classes), dtype=np.float32) for  output_w, output_h in zip(output_ws,output_hs)]
        ...
        for k in range(num_objs):
            ...
            for i in range(anchors_num):
                bbox_xywh_scale = bbox_xywh / self.config.DOWN_RATION[i]
                anchors_xywh = np.zeros((anchors_num, 4))
                anchors_xywh[:, 0:2] = np.floor(bbox_xywh_scale[0:2]).astype(np.int32) + 0.5
                anchors_xywh[:, 2:4] = anchors[i]
                iou_temp=cal_iou(bbox_xywh_scale[np.newaxis,:],anchors_xywh)
                iou.append(iou_temp)
                iou_mask = iou_temp > 0.3
                if np.any(iou_mask):
                    xind, yind = np.floor(bbox_xywh_scale[0:2]).astype(np.int32)
            ...
            if not exist_positive:
                ...
                xind, yind = np.floor( (bbox_xywh / self.config.DOWN_RATION[best_detect])[0:2]).astype(np.int32)
