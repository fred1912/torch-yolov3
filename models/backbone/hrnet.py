...
def hrnet(cfg, is_train=True, **kwargs):
    model = HighResolutionNet(cfg.BACKBONE, **kwargs)
    if is_train and cfg.BACKBONE.INIT_WEIGHTS:
        if isinstance(cfg.BACKBONE.PRETRAINED, str):
            model.init_weights(cfg.BACKBONE.PRETRAINED)
        else:
            raise Exception("hrnet request a pretrained path. got [{}]".format(cfg.BACKBONE.PRETRAINED))
    return model
